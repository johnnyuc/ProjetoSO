...
SharedMemory *init_shared_memory(char *data) {
...
void test_shared_memory() {
    ...
    char data[sizeof(SharedMemory)];
    SharedMemory *sharedMemory = init_shared_memory(data);
    ...
    for (int i = 0; i < sharedMemory->nextFreeNode; i++) {
        ...
        printf("Node ID: %d, Chave: %s, Valor: %.2f\n", node->id, node->chave, node->valor);
    }
